...
export default async function CalendarPage({ params: { locale }, searchParams }: { params: { locale: string }; searchParams: { m?: string; y?: string } }) {
  ...
  const { rows: events } = await pool.query(
    `SELECT id, slug, title_sv, title_en, starts_at, ends_at, venue, published
     FROM events
  ...
  return (
    <div className="min-h-screen bg-[#F9F7F4]">
      <div className="bg-[#58595b] text-white py-12 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <h1 className="text-4xl font-bold">{sv ? 'Kalender' : 'Calendar'}</h1>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            {days.map((day, i) => (
              <div key={i} className={"min-h-[100px] border-b border-r p-2 " + (day ? 'bg-white' : 'bg-gray-50')}>
                {day && (
                  <>
                    ...
                    {(eventsByDay[day] || []).map(e => (
                      <a key={e.id} href={`/${locale}/events/${e.slug}`} className="block text-xs bg-[#f5ca00] text-white rounded px-1.5 py-0.5 mb-1 truncate hover:bg-[#d4af00]">
                        {sv ? e.title_sv : e.title_en}
                      </a>
                    ))}
                  </>
                )}
        ...
        <div className="mt-8">
          ...
          <div className="space-y-3">
            {events.map((e: any) => (
              <a key={e.id} href={`/${locale}/events/${e.slug}`} className="block bg-white rounded-xl border p-4 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-medium text-gray-900">{sv ? e.title_sv : e.title_en}</p>
                    <p className="text-sm text-gray-500">{e.venue || 'Yeshin Norbu, Stockholm'}</p>
                  </div>
                  <div className="text-right text-sm text-gray-500">
                    <p>{new Date(e.starts_at).toLocaleDateString('sv-SE', { weekday: 'short', day: 'numeric', month: 'short' })}</p>
                    <p>{new Date(e.starts_at).toLocaleTimeString('sv-SE', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                </div>
              </a>
            ))}
